...
digital: "🎵", kidsclass: "🎨", car: "🚙", mortgage: "💳", vacation: "✈️", free: "💚",
};
...
const getBucketName = (id) => { if(id==="free_money") return "💚 כסף פנוי"; const b=[...data.fixedBuckets,...data.variableBuckets].find(b=>b.id===id); return b?`${ICONS[b.icon]||"📌"} ${b.name}`:"—"; };
const getBucketType = (id) => { if(id==="free_money") return "כסף פנוי"; if (fixedBucketIds.has(id)) return "קבועה"; if (variableBucketIds.has(id)) return "משתנה"; return "—"; };
...
<Tube label="free" fillPct={totalMonthlyIncome>0?Math.max(0,remaining)/totalMonthlyIncome:0} gradA="#a8d5e2" gradB="#5fa8b8" title="כסף פנוי" sub={`₪${Math.abs(Math.round(remaining)).toLocaleString("he-IL")}`} extra={remaining<0?"גירעון!":null}/>
</div>
...
{remaining>0&&<option value="free_money">💚 כסף פנוי (₪{Math.round(remaining).toLocaleString("he-IL")})</option>}
